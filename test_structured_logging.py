...
def test_structured_logging():
    ...
    if response.status_code == 201:
        print("   ✅ Inscription réussie - Logs générés")
        ...
        if form_response.status_code == 201:
            ...
            if question_response.status_code == 201:
                print("   ✅ Question créée - Logs générés")
                
                # Test 4: Soumission de réponse avec logging
                print("\n4. Test soumission réponse avec logging:")
                
                response_data = requests.post(f"{api_url}/api/forms/{form_id}/responses", json={
                    "answers": {
                        "1": "Réponse de test pour le logging"
                    },
                ...
                if response_data.status_code == 201:
                    print("   ✅ Réponse soumise - Logs générés")
                else:
                    print(f"   ❌ Erreur soumission: {response_data.text}")
            else:
                print(f"   ❌ Erreur création question: {question_response.text}")
        else:
            print(f"   ❌ Erreur création formulaire: {form_response.text}")
    else:
    ...
    print("\n📊 Vérifiez les logs Render pour voir les logs structurés JSON")
...
def test_logging_performance():
    ...
    print("\n📈 Métriques de performance enregistrées dans les logs")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n✅ Tests de logging structuré terminés")
        print("\n🔍 Consultez les logs Render pour voir les logs JSON structurés")
