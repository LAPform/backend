...
def register():
    ...
    try:
        ...
        if not data or "email" not in data or "password" not in data:
            return SecurityAuthManager.create_error_response("Email et mot de passe requis", 400)
        ...
        if not SecurityAuthManager.validate_email(email):
            return SecurityAuthManager.create_error_response("Format d'email invalide", 400)
        ...
        if name and len(name) > 100:
            return SecurityAuthManager.create_error_response("Le nom ne peut pas dépasser 100 caractères", 400)
        ...
        if existing_user:
            return SecurityAuthManager.create_error_response("Un compte avec cet email existe déjà", 409)
        ...
        try:
            ...
            if SecurityAuthManager.login_user_safe(user):
                ...
                return SecurityAuthManager.create_error_response("Erreur lors de la connexion", 500)
                
        except Exception as e:
            logger.error(f"Erreur création utilisateur: {e}")
            return SecurityAuthManager.create_error_response("Erreur lors de la création du compte", 500)
    
    except Exception as e:
        logger.error(f"Erreur inscription: {e}")
        return SecurityAuthManager.create_error_response("Erreur interne du serveur", 500)
...
def login():
    ...
    try:
        ...
        if not user:
            api_logger.authentication_failed(email, "User not found", request.remote_addr)
            return SecurityAuthManager.create_error_response("Identifiants invalides", 401)
        
        # Vérifier le mot de passe
        if not datastore.verify_password(user, password):
            api_logger.authentication_failed(email, "Invalid password", request.remote_addr)
            return SecurityAuthManager.create_error_response("Identifiants invalides", 401)
...
def logout():
    ...
    try:
        if SecurityAuthManager.logout_user_safe():
            ...
            return SecurityAuthManager.create_error_response("Erreur lors de la déconnexion", 500)
...
def get_current_user():
    ...
    try:
        ...
        if user_info:
            ...
            return SecurityAuthManager.create_error_response("Utilisateur non trouvé", 404)
...
def change_password():
    ...
    try:
        ...
        if not data or "current_password" not in data or "new_password" not in data:
            return SecurityAuthManager.create_error_response("Mot de passe actuel et nouveau mot de passe requis", 400)
        ...
        if not datastore.verify_password(current_user, current_password):
            return SecurityAuthManager.create_error_response("Mot de passe actuel incorrect", 401)
