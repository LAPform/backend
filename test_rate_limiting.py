...
def test_rate_limiting():
    ...
    for i in range(7):  # Dépasser la limite
        ...
        print(f"   Requete {i+1}: Status {response.status_code}")

        if response.status_code == 429:
            print(f"   Rate limit atteint: {response.json()}")
            break
        elif response.status_code == 201:
            print(f"   Inscription reussie")
        else:
    ...
    for i in range(12):  # Dépasser la limite
        ...
        if response.status_code == 429:
            ...
            print(f"   Connexion echouee (attendu)")
        else:
            print(f"   Erreur: {response.json()}")
    ...
    for i in range(5):  # Test rapide
        ...
        if response.status_code == 200:
            ...
            print(f"   Requete {i+1}: Erreur {response.status_code}")

        time.sleep(0.5)

    print(f"   Resultat: {success_count}/5 requetes reussies")
    ...
    if response.status_code == 201:
        print("   Headers de rate limiting presents")
    else:
        print("   Headers de rate limiting manquants")

    print("\n" + "=" * 50)
    print("Tests de rate limiting termines")
...
def test_rate_limiting_with_auth():
    ...
    for i in range(5):  # Test modéré
        response = requests.post(
            f"{api_url}/api/forms",
            json={"title": f"Test Form {i}", "description": "Test description"},
            headers=headers,
        )
    ...
    print("\n" + "=" * 50)
    print("Tests de rate limiting avec authentification termines")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nTests de rate limiting termines")
