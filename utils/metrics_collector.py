...
# Import conditionnel de psutil avec gestion d'erreur
try:
    import psutil
    ...
    logging.warning("psutil non disponible - métriques système limitées")
...
class MetricsCollector:
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        indicators = {"status": "healthy", "alerts": [], "warnings": []}

        # Vérifications système (seulement si psutil est disponible)
        if PSUTIL_AVAILABLE and "error" not in system_metrics:
            ...
            indicators["warnings"].append(
                "Métriques système limitées - psutil non disponible"
            )

        # Vérifications API
        if api_metrics.get("error_rate", 0) > 10:
            indicators["alerts"].append(
                f"Taux d'erreur élevé ({api_metrics['error_rate']}%)"
            )
            indicators["status"] = "degraded"

        if api_metrics.get("avg_response_time", 0) > 2:
            indicators["warnings"].append(
                f"Temps de réponse élevé ({api_metrics['avg_response_time']}s)"
            )

        if api_metrics.get("response_time_p95", 0) > 5:
            indicators["warnings"].append(
                f"P95 temps de réponse élevé ({api_metrics['response_time_p95']}s)"
            )
